...
2000.times {
    UserGame.create(user_id: User.all.sample.id,
        ...
        rating: rand(1..100),
        review: Faker::Lorem.paragraph(sentence_count: rand(1..40)),
...
print "SEEDING PRIME ACCOUNT... "
chars = (0...36).map{ |i| i.to_s 36}
me = User.create(name: "sean",
    password: "asdfasdf",
    password_confirmation: "asdfasdf",
    favorite_game: Game.all.sample.id,
    favorite_genre: Genre.all.sample.id,
    favorite_mode: Mode.all.sample.id
    )
20.times {
    UserGame.create(user_id: me.id,
        game_id: Game.all.sample.id,
        rating: rand(1..100),
        review: Faker::Lorem.paragraph(sentence_count: rand(1..40)),
        hours_played: rand(1..100)
